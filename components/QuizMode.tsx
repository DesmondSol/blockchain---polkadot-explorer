...
export const QuizMode: React.FC<QuizModeProps> = ({ learningPath, activeTopic, userExpertise, onMarkTopicComplete }) => {
  ...
  if (!activeTopic && !isLoading && !error && !currentQuiz) {
     return (
  ...
  return (
    <div className="flex flex-col h-full p-2 md:p-4 bg-transparent items-center">
        <div className="mb-2 text-center w-full max-w-2xl shrink-0">
        ...
        {error && !isLoading && <ErrorMessage message={error} onClose={() => { setError(null); setCurrentQuiz(null);}} />}
        
        {isLoading && <div className="flex flex-col items-center justify-center flex-grow p-4"><LoadingSpinner /><p className="mt-2 text-purple-300">{t(Constants.QUIZ_MODE_LOADING_QUIZ_KEY)}</p></div>}

        {!isLoading && !error && !currentQuiz && activeTopic && (
             <div className="flex-grow flex items-center justify-center text-white"><p>{t(Constants.QUIZ_MODE_NO_QUIZ_FOR_TOPIC_KEY)}</p></div>
        )}

        {!isLoading && currentQuiz && (
            <>
                <div className="flex-grow flex flex-col items-center w-full max-w-2xl bg-black bg-opacity-30 backdrop-blur-sm p-4 md:p-6 rounded-lg overflow-y-auto custom-scrollbar mb-4 min-h-0"> {/* Scrollable content area */}
                        ...
                        {currentQuiz.options.map((option: QuizModeOption) => {
                            ...
                            if (showFeedback) {
                                if (option.id === currentQuiz.correctAnswerId) {
                                    ...
                                    buttonClass = 'bg-gray-700 border-gray-600 text-gray-400 opacity-70'; 
                                }
                            } else if (isSelected) {
                ...
                <div className={`w-full max-w-2xl shrink-0 ${showFeedback ? 'mt-0' : 'mt-auto pt-4'}`}> {/* Buttons container */}
                    {!showFeedback && (
                        <IconButton
                            onClick={handleSubmitAnswer}
                            disabled={!selectedAnswerId}
                            className="w-full px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg text-lg disabled:bg-gray-500 disabled:cursor-not-allowed"
                    ...
                    {showFeedback && (
                        <div className={`p-4 rounded-md w-full ${selectedAnswerId === currentQuiz.correctAnswerId ? 'bg-green-800 bg-opacity-70' : 'bg-red-800 bg-opacity-70'}`}>
